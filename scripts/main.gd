...
func _ready() -> void:
	...
	if not has_method("_apply_mode"):
		push_error("Main: _apply_mode() not found. Ensure this script is attached to the Main node.")
		return

	if player == null:
		push_error("Main: Player node not found at 'Player'.")
...
func _apply_mode() -> void:
	...
	if player and player.has_method("set_speed_multiplier"):
		...
		push_warning("Main: Player missing or does not implement set_speed_multiplier.")
	_update_button_text()
